...
global numconn,BUFF,usrp_address,conn,isolationlevels,db_filename,schema_filename,addr,times,fobj,odb_filename,oschema_filename 
numconn = conn() ;times=0
BUFF=4096;usrp_address=['addr=192.168.30.2', 'addr=192.168.20.2']
isolationlevels= 'IMMEDIATE'; db_filename ='sense1.db';schema_filename = 'sensetable.sql';odb_filename ='sense2.db';oschema_filename = 'osensetable.sql'
...
class server_open_port(object):
    ...
    def setupdb(self):
        global conn,isolationlevels,db_filename,schema_filename,odb_filename,oschema_filename 
    ...
    def updatedb(self,data,clientsock):
        ...
        if segmnt=='c1new' or segmnt=='c1old': 
            with sqlite3.connect(db_filename,isolation_level = isolationlevels) as conn:
                queryinsert ="""insert into sense1(usrp,stfreq,tloc,tserv,pwr) values(?,?,?,?,?)"""
                queryupdate ="""update sense1 set tloc=?,tserv=?,pwr=? where stfreq=? and usrp=?"""
        ...
        if segmnt=='c2new' or segmnt=='c2old': 
            with sqlite3.connect(odb_filename,isolation_level = isolationlevels) as conn1:
                queryinsert ="""insert into sense2(usrp,stfreq,tloc,tserv,pwr) values(?,?,?,?,?)"""
                queryupdate ="""update sense2 set tloc=?,tserv=?,pwr=? where stfreq=? and usrp=?"""
                ...
                if segmnt=='c2new':
                    params = data
                    params.pop();params.pop();Address= params.pop();stfreq= params.pop();timelocal= params.pop();power_dbm= params.pop();time_server = datetime.datetime.now() 
                    conn1.execute(queryinsert,(Address,stfreq,timelocal,time_server,power_dbm))
                    print 'Sensing data for client 2 start freq %d inserted\n'%(stfreq)  
      
                elif segmnt=='c2old':
                    params = data;time_server = datetime.datetime.now()  
                    Address= params.pop();stfreq= params.pop();timelocal= params.pop();power_dbm= params.pop()
                    conn1.execute(queryupdate,(timelocal,time_server,power_dbm,stfreq,Address))
